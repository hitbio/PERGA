...
{
	printf("\n============= Begin to construct k-mer hash table by corrected reads, please wait ... =============\n");
	...
	if(AdjustReadsetByCorrectedReads(graph, correctedReadsFile)==FAILED)
	{
		printf("line=%d, In %s(), cannot adjust the read set by corrected reads, error!\n", __LINE__, __func__);
		return FAILED;
	}

	// initialize the k-mer hash table after correction
	if(initGraphAfterCorrection(deBruijnGraph)==FAILED)
	{
		printf("line=%d, In %s(), cannot initialize the k-mer hash table after correction, error!\n", __LINE__, __func__);
		return FAILED;
	}

	// count the k-mer occurrences
	if(countKmerOccsFromReadsetAfterCorrection(graph)==FAILED)
	{
	...
	if(addKmerRidposFromReadsetAfterCorrection(graph)==FAILED)
	{
		printf("line=%d, In %s(), cannot add reads information the kmers from read set, error!\n", __LINE__, __func__);
		return FAILED;
	}

	// output the graph to file
	if(outputGraphToFile(graphFileCorrected, deBruijnGraph)==FAILED)
	{
		printf("line=%d, In %s(), can not output graph to file. Error!\n", __LINE__, __func__);
		return FAILED;
	}
...
/**
 * Count the k-mer occurrences from read set after read correction.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
short countKmerOccsFromReadsetAfterCorrection(graphtype *deBruijnGraph)
{
	int32_t i, j, percent, successFlagPairedArray[2], successFlagPaired;
	readSet_t *readSet;
	readBlock_t *pReadBlockArray;
	read_t *pRead, *pReadArray;
	int64_t rid, totalReadNumTmp;
	...
	for(i=0; i<readSet->blocksNumRead; i++)
	{
		...
		for(j=0; j<pReadBlockArray->itemNum; j++)
		{
			...
			if((rid&1)==1)
			{
				successFlagPairedArray[0] = pRead->successFlag;
				successFlagPairedArray[1] = (pRead + 1)->successFlag;
				successFlagPaired = successFlagPairedArray[1];
			}else
			{
				successFlagPaired = successFlagPairedArray[0];
			}

			//if(pRead->validFlag==YES)
			if(pRead->validFlag==YES && (pRead->successFlag==YES || successFlagPaired==YES))
			{
				//count the kmers
				if(addReadPreFromReadset(pRead, deBruijnGraph)==FAILED)
				{
...
/**
 * Add the k-mer ridpos information from read set after read correction.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
...
short getSingleReadFastq(FILE *fpPE, readBuf_t *pReadBuf)
{
	...
	head_name = pReadBuf->headname;
	pReadBuf->headNameLen = 0;
#endif
	...
	while(!feof(fpPE))
	{
		if(line_index==0)  //the sequence name line
		{
			...
			{
#if(DRAW_CURVE_FLAG==YES)
				head_name[ pReadBuf->headNameLen ++ ] = ch;
#endif
				ch = fgetc(fpPE);
			}
...
/**
 * Initialize k-mer hash table after correction.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
...
short outputGraphToFile(char *graphFile, graphtype *graph)
{
	...
	tmp[0] = readLen;
	tmp[1] = averReadLenInFileSample;
	tmp[2] = kmerSize;
	tmp[3] = hashTableSize;
	tmp[4] = hashTableSizeReadseq;
	tmp[5] = pairedMode;
	tmp[6] = PEGivenType;

	// part (1)-1: readLen, kmerSize, hashTableSize, hashTableSizeReadseq, pairedMode, PEGivenType
	if(fwrite(tmp, sizeof(uint64_t), 7, fpGraph)!=7)
	{
...
short loadGraph(graphtype **graph, char *graphFile)
{
	...
	FILE *fpGraph;
	uint64_t tmp[7], i, j, sum, itemNumKmerBlock;
	double insSdev[2];
	kmertype *kmer;
	...
	// part (1)-1: readLen, kmerSize, hashTableSize, hashTableSizeReadseq, pairedMode, PEGivenType
	if(fread(tmp, sizeof(uint64_t), 7, fpGraph)!=7)
	{
		printf("line=%d, In %s(), fread Error!\n", __LINE__, __func__);
		return FAILED;
	}
	...
	readLen = tmp[0];
	averReadLenInFileSample = tmp[1];
	kmerSize = tmp[2];
	hashTableSize = tmp[3];
	hashTableSizeReadseq = tmp[4];
	pairedMode = tmp[5];
	PEGivenType = tmp[6];
 ...
 */
short GlobalParasFromGraph(int *readLenPara, int *averReadLenInFilePara, int *kmerSizePara, uint64_t *hashTableSizePara,
		uint64_t *hashTableSizeReadseqPara, int *pairedModePara, int32_t *PEGivenTypePara,
		double *meanSizeInsertPara, double *standardDevPara, char *graphFileName)
{
	...
	*readLenPara = tmp[0];
	*averReadLenInFilePara = tmp[1];
	*kmerSizePara = tmp[2];
	*hashTableSizePara = tmp[3];
	*hashTableSizeReadseqPara = tmp[4];
	*pairedModePara = tmp[5];
	*PEGivenTypePara = tmp[6];
...
short updateInsertSizeAndSdevInGraph(int32_t PEGivenType, double meanSizeInsert, double standardDev, const char *graphFileName)
{
	...
	if(fseek(fpGraph, 6*sizeof(uint64_t), SEEK_SET)==-1)
	{
...
{
	int32_t i, j, pos, basePos, startKmerPos, startBasePos, endBasePos, endKmerNum;
	int32_t baseInt, entriesNum, baseNumLastEntry, entryRow, entryPos;
	uint64_t hashcode;
