...
short updatePEHashTable(int contigNodesNum, int assemblyRound)
{
	...
	{ // slide the window by one base pair
		...
		if(assemblyRound==FIRST_ROUND_ASSEMBLY)
		{ // the first round
			//shiftedRegLeftContig ++;
			//shiftedRegRightContig ++;
	...
	}else if(contigNodesNum>=minMarginLenPEHash)
	{
		...
		{ // initialize the PE hash table
			...
			{ // the first round
				...
				//shiftedRegLeftContig = shiftedRegRightContig = contigArr + newContigIndex - 1;
			...
			{ // the second round
				//hashRegLeftContig = hashRegRightContig = contigArr;
