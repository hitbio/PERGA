...
short contigsLenStatistics(int32_t *contigsLenArray, int32_t itemNumContigsLenArray)
{
	...
	if(radixSortContigsLen(contigsLenArray, contigsLenBufTmp, itemNumContigsLenArray)==FAILED)
	{
	...
	if(computeLenStatistics(contigsLenArray, itemNumContigsLenArray)==FAILED)
	{
...
short computeLenStatistics(int32_t *contigsLenArray, int32_t itemNumContigsLenArray)
{
	...
	printf("There are %d contigs with total length %ld, maximal Size is %d, \n"
			"N50 size %d, mean Size %.2f, median size %ld.\n",
			itemNumContigsLenArray, totalQueryLen, contigsLenArray[0],
