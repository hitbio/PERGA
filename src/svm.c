...
short loadSvmModel(svmModel_t **svmModel, char *svmKernelFuncName, double *supportVector, double *alphaVector, double biasData, double *scaleData, int32_t rowsNumSV, int32_t colsNumSV, int32_t useScaleDataFlag)
{
	...
	if(initMemSvmModel(svmModel, rowsNumSV, colsNumSV, useScaleDataFlag)==FAILED)
	{
	...
	strcpy((*svmModel)->kernelFunc, svmKernelFuncName);

	// load the support vectors
	if(loadSupportVectorData(*svmModel, supportVector)==FAILED)
	{
	...
	if(loadAlphaVectorData(*svmModel, alphaVector)==FAILED)
	{
	...
	// set the bias
	(*svmModel)->bias = biasData;

	// load the scale data
	if(loadScaleData(*svmModel, scaleData)==FAILED)
	{
...
short initMemSvmModel(svmModel_t **svmModel, int32_t rowsNumSV, int32_t colsNumSV, int32_t useScaleDataFlag)
{
	...
	*svmModel = (svmModel_t *) calloc (1, sizeof(svmModel_t));
	...
	(*svmModel)->rowsNumSupportVector = rowsNumSV;
	(*svmModel)->colsNumSupportVector = colsNumSV;

	// allocate memory for support vectors
	(*svmModel)->supportVectors = (svmSupportVector_t*) calloc(rowsNumSV, sizeof(svmSupportVector_t));
	...
	for(i=0; i<rowsNumSV; i++)
	{
		(*svmModel)->supportVectors[i].vectorData = (double*) malloc(sizeof(double)*colsNumSV);
...
short loadSupportVectorData(svmModel_t *svmModel, double *supportVector)
{
	...
	for(i=0; i<rowsNum; i++)
		for(j=0; j<colsNum; j++)
			svmModel->supportVectors[i].vectorData[j] = supportVector[i*colsNum+j];
...
short loadAlphaVectorData(svmModel_t *svmModel, double *alphaVector)
{
	...
	for(i=0; i<svmModel->rowsNumAlphaVector; i++)
		svmModel->alphaVector[i] = alphaVector[i];

	return SUCCESSFUL;
}

/**
 * Load the scale data from file.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
short loadScaleData(svmModel_t *svmModel, double *scaleData)
{
	...
	if(svmModel->scaleData)
	{
		...
		for(i=0; i<2; i++)
		{
			...
			for(j=0; j<colsNum; j++)
				pVector[j] = scaleData[i*colsNum+j];
		}
	}
...
{
	// initialize the memory for the SVM classifier model
	if(initMemSvmModelFromFile(svmModel, supportVectorFile, scaleDataFile)==FAILED)
	{
		printf("line=%d, In %s(), cannot initialize the memory for SVM model, error!\n", __LINE__, __func__);
		return FAILED;
	}
	...
	if(loadKernelFuncNameFromFile(*svmModel, kernelFuncName)==FAILED)
	{
		printf("line=%d, In %s(), cannot load the support vectors, error!\n", __LINE__, __func__);
		return FAILED;
	}
	...
	// load the support vectors
	if(loadSupportVectorDataFromFile(*svmModel, supportVectorFile)==FAILED)
	{
		printf("line=%d, In %s(), cannot load the support vectors, error!\n", __LINE__, __func__);
		return FAILED;
	}
	...
	// load the Alpha vector data
	if(loadAlphaVectorDataFromFile(*svmModel, alphaFile)==FAILED)
	{
		printf("line=%d, In %s(), cannot load the alpha vectors, error!\n", __LINE__, __func__);
		return FAILED;
	}
	...
	// load the bias
	if(loadBiasDataFromFile(*svmModel, biasFile)==FAILED)
	{
	...
	// load the scale data
	if(loadScaleDataFromFile(*svmModel, scaleDataFile)==FAILED)
	{
		printf("line=%d, In %s(), cannot load the scale data, error!\n", __LINE__, __func__);
		return FAILED;
	}
...
/**
 * Initialize the memory for sample data before using SVM classifier from file.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
short initMemSvmModelFromFile(svmModel_t **svmModel, char *supportVectorFile, char *scaleDataFile)
{
	...
	*svmModel = (svmModel_t *) calloc (1, sizeof(svmModel_t));
	...
	// get rows number and columns number
	if(getRowsColsNumSupportVectorsFromFile(&((*svmModel)->rowsNumSupportVector), &((*svmModel)->colsNumSupportVector), supportVectorFile)==FAILED)
	{
...
/*
	...
	svmSample->colsNum = colsNumSupportVector;
	svmSample->vectorData = (double *) malloc(colsNumSupportVector*sizeof(double));
...
*/
	...
	svmSampleSE->colsNum = colsNumSupportVector;
	svmSampleSE->vectorData = (double *) malloc(colsNumSupportVector*sizeof(double));
	...
	svmSamplePE->colsNum = colsNumSupportVector;
	svmSamplePE->vectorData = (double *) malloc(colsNumSupportVector*sizeof(double));
...
{
/*
	free(svmSample->vectorData);
	svmSample->vectorData = NULL;
	free(svmSample);
	svmSample = NULL;
*/

	free(svmSampleSE->vectorData);
	svmSampleSE->vectorData = NULL;
	free(svmSampleSE);
	svmSampleSE = NULL;

	free(svmSamplePE->vectorData);
	svmSamplePE->vectorData = NULL;
	free(svmSamplePE);
	svmSamplePE = NULL;
}
...
/**
 * Load the kernel function name from file.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
...
/**
 * Load the support vectors data from file.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
...
/**
 * Load the alpha vectors data from file.
 *  @return:
 *  	If succeeds, return SUCCESSFUL; otherwise, return FAILED.
 */
